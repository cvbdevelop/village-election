...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const voters = await client.query('SELECT voted FROM voters WHERE id = $1', [voterId]);
    ...
    await client.query('INSERT INTO votes (candidate_id, voter_id) VALUES ($1, $2)', [candidateId, voterId]);
    await client.query('UPDATE voters SET voted = TRUE WHERE id = $1', [voterId]);
...
router.get('/results', async (req, res) => {
  try {
    const result = await db.query(`
      SELECT c.id, c.name, c.party_role AS party, c.number, c.photo, COUNT(v.id)::int AS votes
      FROM candidates c
      LEFT JOIN votes v ON c.id = v.candidate_id
      GROUP BY c.id
      ORDER BY votes DESC
    `);
    const totalVotes = result.rows.reduce((sum, r) => sum + r.votes, 0);
